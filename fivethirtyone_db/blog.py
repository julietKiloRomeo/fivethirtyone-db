...
def index():
    ...
    lifts = sorted(db.all_sets(athlete=g.user), key = lambda lift: lift["date"] or way_in_the_future, reverse=True)
    # [
...
def make_pdf():
    ...
    compile = lambda ws: analysis.compile(g.user, ws["lift_name"], ws["base_max"]*0.9, programs[ws["base_reps"]], cycle=ws["cycle"])
    program = {ws["lift_name"]:compile(ws) for ws in worksets_to_do}
    program = [{"lift":lift, "reps":df.loc["reps"].to_list(), "weight":df.loc["weight"].to_list(), } for lift, df in program.items()]
    print(program)
...
def estimate_next_cycle(athlete):
    ...
    latest_max_1rm = { lift: analysis.one_rm_fusion(ws["weight"], ws["reps"]) for lift, ws in latest_max_set.items() }
